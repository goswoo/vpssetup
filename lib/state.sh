...
state_allowed_key() {
    case "$1" in
        ...
        RELEASE_REPO|MANAGED_SWAPFILE|IPV6_METHOD|IPV6_UFW_PREVIOUS|\
...
load_state() {
    ...
    validate_port "$SSH_PORT" || SSH_PORT="60600"
    validate_repo_slug "$RELEASE_REPO" || RELEASE_REPO=""
    ...
    for boolean_key in \
        ...
        SUDO_TIMEOUT_ENABLED DOCKER_GROUP_ADDED ICMP_LIMIT_ENABLED; do
        [[ "${!boolean_key}" == "true" ]] ||
...
ICMP_LIMIT_ENABLED='${ICMP_LIMIT_ENABLED}'
"
